            ...
            if not hitsWall:
                if (nextx,nexty) in self.corners:
                    if (nextx,nexty) not in foundCorners:
                        seen = foundCorners + [(nextx,nexty)]
                        successors.append((((nextx,nexty),seen),action,1))
...
def cornersHeuristic(state, problem):
    ...
    while unseen:
        distance,corner = min([(util.manhattanDistance(node,corner),corner) for corner in unseen])
